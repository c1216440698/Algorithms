...
void rearrange(int* rev,int length){
    for(int i=0;i<lim;i++){
...
int main(){
    cout<<"Please enter two coefficients."<<endl;
    int n,m;scanf("%d %d",&n,&m);
    cout<<"Please enter the first Polynominal coefficients"<<endl;
    for(int i=0;i<=n;i++) cin>>G[i].r;
    cout<<"Please enter the second Polynomial coefficients"<<endl;
    for(int i=0;i<=m;i++) cin>>F[i].r;
    while(lim<=n+m) {
        lim <<= 1;
        len++;
    }
